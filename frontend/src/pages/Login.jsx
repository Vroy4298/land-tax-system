          ...
          <div className="relative z-10 space-y-6">
            {[
              ...
              <div key={i} className="flex gap-4">
                ...
                <div><h4 className="font-semibold text-slate-100">{item.title}</h4><p className="text-xs text-slate-500">{item.desc}</p></div>
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col justify-center bg-white dark:bg-[#020617] p-8 sm:p-16 lg:p-24 transition-colors">
          <motion.div ref={cardRef} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium"
                  />
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-12 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium"
                  />
              ...
              <div className="flex items-center gap-3 py-1 ml-1">
                <label className="relative flex items-center cursor-pointer group">
                  ...
                  <span className="ml-3 text-sm font-medium text-slate-600 dark:text-slate-400">Stay logged in</span>
                </label>
              </div>

              <Turnstile ref={turnstileRef} siteKey="0x4AAAAAACHZnqi3De6KSsC2" onSuccess={(token) => setCfToken(token)} onError={() => setCfToken(null)} options={{ theme: "auto", size: "invisible" }} />

              <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} disabled={loading} type="submit"
                className={`w-full py-4 rounded-2xl text-white font-semibold text-lg shadow-xl transition-all flex items-center justify-center gap-2 ${loading ? 'bg-slate-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500 shadow-blue-500/25'}`}
              >
            ...
            <div className="mt-12 text-center">
              <p className="text-slate-500 dark:text-slate-400 font-medium">New to the system? <Link to="/register" className="text-blue-600 dark:text-blue-400 font-bold hover:underline">Create an account</Link></p>
            </div>
          </motion.div>
        </div>
      </div>
